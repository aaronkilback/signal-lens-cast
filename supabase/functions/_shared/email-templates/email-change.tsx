...
export const EmailChangeEmail = ({
  ...
  <Html lang="en" dir="ltr">
    <Head />
    <Preview>Confirm credential update for AEGIS</Preview>
    <Body style={main}>
      <Container style={container}>
        <Img
          ...
          style={logo}
        />
        <Heading style={h1}>Confirm Credential Update</Heading>
        <Text style={text}>
          A request was made to update your AEGIS credentials from{' '}
          <Link href={`mailto:${email}`} style={link}>{email}</Link>{' '}
          to{' '}
          <Link href={`mailto:${newEmail}`} style={link}>{newEmail}</Link>.
        </Text>
        <Text style={text}>Confirm this change using the secure link below.</Text>
        <Button style={button} href={confirmationUrl}>
          Confirm Update
        </Button>
        <Text style={footer}>
          If you did not initiate this change, secure your account immediately.
        </Text>
